...
class NLIDataset(Dataset):
    ...
    def __init__(self,
                 data,
                 padding_idx=0,
                 max_premise_length=350,
                 max_hypothesis_length=350):
        """
        Args:
            ...
            max_hypothesis_length: An integer indicating the maximum length
                ...
                Defaults to None.
        """
        self.premises_lengths = [len(seq) for seq in data["passages"]]
        ...
        self.hypotheses_lengths = [len(seq) for seq in data["querys"]]
        ...
        self.num_sequences = len(data["passages"])
        ...
        self.data = {"premises": torch.ones((self.num_sequences,
                     ...
                     "labels": torch.tensor(data["alternatives"], dtype=torch.float)}

        for i, premise in enumerate(data["passages"]):
            end = min(len(premise), self.max_premise_length)
            self.data["premises"][i][:end] = torch.tensor(premise[:end])

            hypothesis = data["querys"][i]
