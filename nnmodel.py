...
class NLINet(nn.Module):
    def __init__(self, config):
        ...
        self.encoder_type = 'InferSent'
        # self.encoder_type = 'InnerAttentionYANGEncoder' #50.84,50.94
        # self.encoder_type = 'InnerAttentionMILAEncoder'
        # self.encoder_type = 'InnerAttentionNAACLEncoder'
        # self.encoder_type ='BGRUlastEncoder' #45
        # self.encoder_type ='BLSTMprojEncoder' #49.73

        # self.encoder_type = 'ConvNetEncoder' #44.27
        # self.encoder_type = 'LSTMEncoder' #
        # self.encoder_type = 'GRUEncoder' #44.27
        self.dpout_fc = config['dpout_fc']

        self.encoder = eval(self.encoder_type)(config)
        self.inputdim = 13*2*self.enc_lstm_dim
        ...
        self._projection = nn.Sequential(nn.Linear(2*2*self.enc_lstm_dim,
                                                        2*self.enc_lstm_dim),
                                                nn.ReLU())
    ...
    def forward(self, s1, s2,aembedding):
        # s1 : (s1, s1_len)
        u = self.encoder(s1)
        v = self.encoder(s2)

        enhanced_premises= torch.cat((u, u*u), 1)
        enhanced_hypotheses= torch.cat((v, v*v), 1)
        enhanced_hypotheses2= torch.cat((torch.abs(u-v), u*v), 1)


        v_ai = self._projection(enhanced_premises)
        v_bj = self._projection(enhanced_hypotheses)
        v_bk = self._projection(enhanced_hypotheses2)


        features = torch.cat([v_ai, v_bj, torch.abs(v_ai-v_bj), v_ai*v_bj,u,v,torch.abs(u-v),u*v,v_ai*v,torch.abs(v_ai-v),
                              v_bj*u,torch.abs(v_bj-u),v_bk], 1)
        ...
        # print 'loss update'
        output = self.classifier(features).view(features.size()[0],1,self.fc_dim)
        # score = F.softmax(aembedding.bmm(output.transpose(2, 1)).squeeze(),1)
        score = F.softmax(aembedding.bmm(output.transpose(2, 1)).squeeze())
        loss = -torch.log(score[:, 0]).mean()
        # pred_index=torch.argmax(score,1)
        pred_index=torch.max(score,1)[1]
        # print pred_index
        # print pred_index
        # print pred_index
        # print loss
