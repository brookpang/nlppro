...
def train(model,
    ...
    for batch_index, batch in enumerate(dataloader):
        ...
        loss,pred_index = model(premises,
                              premises_lengths,
                              hypotheses,
                              hypotheses_lengths,labels)


        # score=torch.cat([criterion(output,labels.select(index=i,dim=1).view(output.size())) for i in range(3)],1)
        # print score.size()
        ...
        correct_preds += (pred_index.cpu().data.numpy()== 0).sum()
...
def validate(model, dataloader, criterion):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            # print output.size()
            # print labels.select(index=0,dim=1).view(output.size()).size()

            # score=torch.cat([criterion(output,labels.select(index=i,dim=1).view(output.size())) for i in range(3)],1)
            # print score.size()
            # print 'score mean,', score[:, 0].mean()
            # loss=-torch.log(score[:, 0]).mean()
            # print 'losss:', loss
            # pred_index=torch.max(score,1)[1]
            ...
            running_accuracy +=  (pred_index.cpu().data.numpy()== 0).sum()
